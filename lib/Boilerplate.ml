...
let map_contextual_keywords (env : env) (x : CST.contextual_keywords) =
  ...
  | `Nameof tok -> R.Case ("Nameof",
      (* "nameof" *) token env tok
    )
...
and map_expression (env : env) (x : CST.expression) =
  ...
  | `Lambda_exp (v1, v2, v3, v4, v5) -> R.Case ("Lambda_exp",
      let v1 =
        (match v1 with
        | Some tok -> R.Option (Some (
            (* "async" *) token env tok
...
and map_function_pointer_parameter (env : env) ((v1, v2) : CST.function_pointer_parameter) =
  let v1 =
    ...
    | Some x -> R.Option (Some (
        ...
        | `Ref_read (v1, v2) -> R.Case ("Ref_read",
            let v1 = (* "ref" *) token env v1 in
...
and map_parameter (env : env) (x : CST.parameter) =
  (match x with
  | `Rep_attr_list_opt_choice_ref_opt_type_id_opt_equals_value_clause (v1, v2, v3, v4, v5) -> R.Case ("Rep_attr_list_opt_choice_ref_opt_type_id_opt_equals_value_clause",
      let v1 = R.List (List.map (map_attribute_list env) v1) in
      let v2 =
        (match v2 with
        | Some x -> R.Option (Some (
...
and map_pattern (env : env) (x : CST.pattern) =
  ...
  | `Bin_pat x -> R.Case ("Bin_pat",
      map_binary_pattern env x
    )
...
let map_record_base (env : env) (x : CST.record_base) =
  (match x with
  | `COLON_id_rep_COMMA_id (v1, v2, v3) -> R.Case ("COLON_id_rep_COMMA_id",
      let v1 = (* ":" *) token env v1 in
      let v2 = map_identifier env v2 in
      let v3 =
        R.List (List.map (fun (v1, v2) ->
  ...
  | `COLON_prim_cons_base_type_opt_COMMA_id_rep_COMMA_id (v1, v2, v3) -> R.Case ("COLON_prim_cons_base_type_opt_COMMA_id_rep_COMMA_id",
      let v1 = (* ":" *) token env v1 in
...
and map_declaration (env : env) (x : CST.declaration) =
  ...
  | `Op_decl (v1, v2, v3, v4, v5, v6, v7) -> R.Case ("Op_decl",
      let v1 = R.List (List.map (map_attribute_list env) v1) in
      let v2 = R.List (List.map (map_modifier env) v2) in
      let v3 = map_type_pattern env v3 in
      let v4 = (* "operator" *) token env v4 in
